...
var Nrf905 = function(options) {
	...
	pin_DR.mode('in');
	var pin_PWR_UP = gpio.connect(options.pin_PWR_UP || 16);
	pin_PWR_UP.mode('out');
	pin_PWR_UP.value(true);
	var pin_TRX_CE = gpio.connect(options.pin_TRX_CE || 15);
	pin_TRX_CE.mode('out');
	pin_TRX_CE.value(false);
	var pin_TX_EN = gpio.connect(options.pin_TX_EN || 11);
	pin_TX_EN.mode('out');
		...
		self.writeConfig([ data.length ], 4, function(status, buf) {
			...
			spiWrite(txbuf, function() {
				var callback = function() {
					pin_TRX_CE.value(false);
					pin_DR.removeListener('rise', callback);
					cb();
				};
				pin_DR.on('rise', callback);
